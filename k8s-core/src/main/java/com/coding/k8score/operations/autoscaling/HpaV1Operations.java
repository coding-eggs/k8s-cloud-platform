...
/**
 * HPA 操作（autoscaling/v1）。由 {@code KubernetesOperationsFactory.build()} 在集群 capability 仅含 autoscaling/v1 时构造。
 */
@Slf4j
@RequiredArgsConstructor
public class HpaV1Operations implements NamespacedOperations<HpaDTO> {

    public final static String apiVersion = "autoscaling/v1";
    ...
    public List<HpaDTO> list(String namespace, String labelSelector, String fieldSelector) {
        ...
        List<HorizontalPodAutoscaler> items = client.autoscaling().v1().horizontalPodAutoscalers()
    ...
    public HpaDTO get(String namespace, String name) {
        ...
        HorizontalPodAutoscaler hpa = client.autoscaling().v1().horizontalPodAutoscalers()
    ...
    public HpaDTO create(HpaDTO dto) {
        ...
        HorizontalPodAutoscaler created = client.autoscaling().v1().horizontalPodAutoscalers()
    ...
    public HpaDTO update(HpaDTO dto) {
        ...
        HorizontalPodAutoscaler in = converter.convert(dto);
        HorizontalPodAutoscaler updated = client.autoscaling().v1().horizontalPodAutoscalers()
    ...
    private boolean checkExist(String namespace, String name) {
        HorizontalPodAutoscaler existing = client.autoscaling().v1().horizontalPodAutoscalers()
                .inNamespace(namespace).withName(name).get();
