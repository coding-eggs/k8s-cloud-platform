...
/**
 * ServiceMonitor DTO（monitoring.coreos.com/v1 CRD）。
 * spec 顶层字段：selector（必填）+ endpoints（必填）+ namespaceSelector / jobLabel /
...
public class ServiceMonitorDTO extends BaseResources {
    ...
    /** 目标 Service 选择器 matchLabels（spec.selector，必填）：由后端据 serviceRef 解析填充；查询时原样返回 */
    private Map<String, String> matchLabels;

    /** 目标 Service 选择器 matchExpressions（spec.selector.matchExpressions）：与 matchLabels 并存，ANDed；功能补全，支持按表达式选 Service */
    private List<MatchExpression> matchExpressions;

    /** 绑定的目标 Service（平台侧）：前端选择后据此反查其 labels 生成 matchLabels；非 K8s 字段，转发 k8s-server 前剥离、不落库 */
    ...
    /** 从目标 Service 的标签透传到目标的 label 列表 */
    ...
    /** 绑定的目标 Service（name + namespace）：namespace 缺省取本资源所在命名空间 */
    @Data
