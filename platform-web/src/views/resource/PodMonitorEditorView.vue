...
interface EndpointRow {
  ...
  bearerName: string
  bearerKey: string
  honorLabels: boolean // 标签冲突时以指标自带标签为准（PodMonitor 特有）
  tlsEnabled: boolean
...
function newEndpoint(): EndpointRow {
  return {
    ...
    bearerEnabled: false, bearerName: '', bearerKey: '', honorLabels: false,
...
const form = reactive({
  ...
  matchLabels: {} as Record<string, string>, // 回显用（只读展示）；后端据 podRef 解析覆盖
  podRef: null as { name: string; namespace: string } | null, // 选择的目标 Pod（覆盖选择器）
  matchExpressions: [] as MatchExpressionRow[], // spec.selector.matchExpressions：按表达式选 Pod（与 matchLabels ANDed）
...
function fromRelabeling(r: K8sPmRelabeling): RelabelingRow {
  return {
...
function fromEndpoint(e: K8sPmEndpoint): EndpointRow {
  const base = newEndpoint()
  base.port = e.port ?? ''
  base.honorLabels = e.honorLabels === true
...
async function loadDetail(): Promise<void> {
  ...
  try {
    const d = await podMonitorApi.get(editing.value, ctx3.value)
    form.name = d.name
    form.labels = { ...(d.labels ?? {}) }
    form.matchLabels = { ...(d.matchLabels ?? {}) }
    form.podRef = null // 编辑时仅回显选择器；重新选择 Pod 才覆盖
    ...
    const eps = (d.podMetricsEndpoints ?? []).map(fromEndpoint)
    form.podMetricsEndpoints = eps.length ? eps : [newEndpoint()]
...
async function loadDiscoveryLabels(): Promise<void> {
  if (!editing.value || !ready.value) return
  try {
    discovery.value = await podMonitorRelabelApi.labels({
...
// ---------- MetricRelabeling regex 的 __name__（指标名）候选（编辑态，scoped 到本 PodMonitor 活跃 target） ----------
const metricNameOptions = ref<string[]>([])
async function loadMetricNames(): Promise<void> {
  if (!editing.value || !ready.value) return
  try {
    metricNameOptions.value = await podMonitorRelabelApi.metricNames({
      clusterId: state.clusterId!, namespace: state.namespace!, name: editing.value,
    }) ?? []
  } catch {
...
const REPLACE_TOKENS = [
  ...
  { token: 'monitor', label: 'PodMonitor 名', ph: '{{monitor}}', resolve: () => form.name || editing.value || '' },
  { token: 'namespace', label: '命名空间', ph: '{{namespace}}', resolve: () => state.namespace ?? '' },
  { token: 'pod', label: '目标 Pod 名（需已绑定）', ph: '{{pod}}', resolve: () => form.podRef?.name ?? '' },
]
...
const RELABEL_REGEX_PRESETS = [
  commonItems,
  {
    ...
    items: [
      ...
      { pattern: '__meta_kubernetes_service_label_(.*)', desc: 'Service label（Pod 发现也带）' },
...
/** MetricRelabeling regex 的指标名候选（el-autocomplete）：按输入前缀/子串过滤本 PodMonitor 活跃 target 的真实 __name__，最多 50 条；无候选时输入框仍可手输 */
...
function assembleRelabelings(rows: RelabelingRow[]): K8sPmRelabeling[] {
  const out: K8sPmRelabeling[] = []
  for (const r of rows) {
    const item: K8sPmRelabeling = {}
...
function assembleEndpoint(e: EndpointRow): K8sPmEndpoint {
  const ep: K8sPmEndpoint = { port: e.port.trim() }
  ...
  if (e.basicAuthEnabled) {
    const ba: NonNullable<K8sPmEndpoint['basicAuth']> = {}
  ...
  if (e.tlsEnabled) {
    const tls: NonNullable<K8sPmEndpoint['tlsConfig']> = {}
...
async function submit(): Promise<void> {
  ...
  // #5：创建必选目标 Pod；编辑可沿用回显的选择器（未改则原样保留），或重新选择覆盖；也可用 matchExpressions 表达式
  const hasExprs = form.matchExpressions.some((m) => m.key.trim())
  if (!form.podRef?.name && !Object.keys(form.matchLabels).length && !hasExprs) { ElMessage.warning('请选择要抓取的目标 Pod（或添加选择器表达式）'); return }

  const pmEndpoints: K8sPmEndpoint[] = []
  for (const e of form.podMetricsEndpoints) {
    if (!e.port.trim()) { ElMessage.warning('每个端点都需填写 Port'); return }
    pmEndpoints.push(assembleEndpoint(e))
  }
  if (!pmEndpoints.length) { ElMessage.warning('至少需要一个抓取端点（spec.podMetricsEndpoints，Port 必填）'); return }

  const body: K8sPodMonitor = { name, namespace: state.namespace!, labels: form.labels, matchLabels: form.matchLabels, podMetricsEndpoints: pmEndpoints }
  if (form.podRef?.name) body.podRef = form.podRef

  // spec.selector.matchExpressions：按表达式选 Pod（与 matchLabels ANDed）；Exists/DoesNotExist 不带 values
  const exprs: K8sPmMatchExpression[] = form.matchExpressions
    .filter((m) => m.key.trim())
    .map((m) => {
      const e: K8sPmMatchExpression = { key: m.key.trim(), operator: m.operator }
  ...
  try {
    if (editing.value) {
      await podMonitorApi.update(editing.value, ctx2.value, body)
      ElMessage.success('保存成功')
    } else {
      await podMonitorApi.create(ctx2.value, body)
      ElMessage.success('创建成功')
    }
    router.push('/resources/podmonitors')
  } catch {
    /* 拦截器已提示 */
  } finally {
    saving.value = false
...
function goBack(): void { router.push('/resources/podmonitors') }

const pageTitle = computed(() => (editing.value ? '编辑 PodMonitor' : '创建 PodMonitor'))
...
<template>
  <div class="res-editor">
    ...
    <EmptyState v-if="!ready" title="尚未选择上下文" description="请在顶栏依次选择租户、集群、命名空间后，再创建或编辑 PodMonitor。" />

    <template v-else>
      <div v-if="formVisible" class="editor-body">
        <!-- 基础信息 -->
        <el-card shadow="never" class="sec-card">
          <template #header><span class="sec-title">{{ editing ? `PodMonitor · ${form.name}` : '基础信息' }}</span></template>
        ...
        <el-card shadow="never" class="sec-card">
          ...
          <el-form label-width="200px" label-position="left">
            <el-form-item label="目标 Pod" required>
              <template #label>目标 Pod <FieldHelp tip="选择要抓取的目标 Pod；后端据其 labels 自动生成选择器（spec.selector）。编辑时重新选择即可覆盖。" /></template>
              <el-select v-model="podSelectValue" filterable clearable :loading="podLoading" placeholder="选择目标 Pod（必填）" style="width: 420px">
                <el-option
                  v-for="p in podOptions"
                  :key="`${p.namespace}/${p.name}`"
                  :label="targetNamespaces.length > 1 ? `${p.name} · ${p.namespace}` : p.name"
                  :value="`${p.namespace}/${p.name}`"
                />
              </el-select>
            </el-form-item>
            <el-form-item label="选择器（只读）">
              <template #label>选择器 <FieldHelp tip="spec.selector.matchLabels：由所选 Pod 的 labels 决定，不可手改；重新选择 Pod 才会覆盖。" /></template>
              ...
              <span v-else-if="form.podRef?.name" class="muted">将使用 Pod「{{ form.podRef.name }}」的 labels 生成</span>
              <span v-else class="muted">尚未选择（创建时必选目标 Pod）</span>
            </el-form-item>
            <el-form-item label="选择器表达式">
              <template #label>选择器表达式 <FieldHelp tip="spec.selector.matchExpressions：按标签表达式选 Pod，与 matchLabels ANDed。Exists / DoesNotExist 无需填值。" /></template>
              <div class="expr-list">
            ...
            <el-form-item label="命名空间范围">
              <template #label>命名空间范围 <FieldHelp tip="spec.namespaceSelector：留空 = 仅本命名空间；勾选多个后跨所选命名空间抓取，并据此加载 Pod 列表。" /></template>
              <div class="ns-editor">
        ...
        <!-- 抓取端点 podMetricsEndpoints -->
        <el-card shadow="never" class="sec-card">
          <template #header><span class="sec-title">抓取端点（podMetricsEndpoints）</span></template>
          <div v-for="(e, idx) in form.podMetricsEndpoints" :key="idx" class="ep-block">
            <div class="ep-head">
              <span class="ep-title">端点 #{{ idx + 1 }}</span>
              <el-button link type="danger" :disabled="form.podMetricsEndpoints.length <= 1" @click="form.podMetricsEndpoints.splice(idx, 1)">删除端点</el-button>
            </div>
            ...
            <div class="ep-grid">
              ...
              <div class="field">
                <label>Port * <FieldHelp tip="目标 Pod 的容器端口（名字或端口号）；从所选 Pod 选择，也可手输。提交时纯数字→portNumber，命名→port。" /></label>
                <el-select v-model="e.port" filterable allow-create clearable :placeholder="form.podRef ? '选择端口' : '先选择目标 Pod'" no-data-text="无匹配端口">
            ...
            <el-collapse class="ep-adv">
              ...
              <el-collapse-item title="鉴权 / TLS / 标签行为" name="auth">
              ...
              <el-collapse-item :title="`MetricRelabeling（${e.metricRelabelings.length}）`" name="mrel">
                ...
                <div v-for="(r, ri) in e.metricRelabelings" :key="ri" class="rel-block">
                  ...
                  <div class="rel-field">
                    <label>regex<FieldHelp tip="匹配正则（RE2，留空=默认 (.*)）。作用于 sourceLabels 拼接值；labelmap/labeldrop/labelkeep 作用于标签名。输入时可从本 PodMonitor 真实暴露的指标名（__name__）里选，也可手输；右侧「常用正则」可选常用写法，选后仍可继续改。" /></label>
                    <div class="rel-input-wrap">
          ...
          <el-button class="add-ep-btn" plain @click="form.podMetricsEndpoints.push(newEndpoint())">+ 添加端点</el-button>
        </el-card>
        ...
        <el-card shadow="never" class="sec-card">
          ...
          <el-form label-width="200px" label-position="left">
            <el-form-item label="podTargetLabels">
              <template #label>podTargetLabels <FieldHelp tip="从目标 Pod 的标签透传到抓取目标的 label 列表。" /></template>
      ...
      <EmptyState v-else-if="detailState === 'error'" title="加载 PodMonitor 失败" description="请返回列表重试；若该资源已被删除，刷新列表即可。">
